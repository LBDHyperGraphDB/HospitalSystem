    ...
    public boolean addDoctor(Doctor doctor) {
        try {
            ...
            if (!this.findDoctorByCRM(hospitalGraph, doctor.getDoctorCrm())) {
                ...
                System.out.println("[ERRO]: O CRM " + doctor.getDoctorCrm() + " jÃ¡ existe.");
                return true;
            }
        } catch (Throwable t) {
            System.out.println("[ERRO]: O médico " + doctor.getPersonName() + " nÃ£o pÃ´de ser adicionado.");
            t.printStackTrace();
            return false;
        } finally {
            hospitalGraph.close();
    ...
    public void getAllDoctors() {
        try {
            ...
            if (doctors.size() > 0) {
                ...
                System.out.print("NÃ£o hÃ¡ médicos cadastrados.");
            }
            System.out.println();
        } catch (Throwable t) {
            t.printStackTrace();
